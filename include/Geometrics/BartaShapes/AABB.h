...
namespace Barta{
	...
	*/
	...
	inline AABB::VoronoiRegion operator | (const AABB::VoronoiRegion& first, const AABB::VoronoiRegion& second) noexcept {
	...
	inline AABB::VoronoiRegion operator ~ (const AABB::VoronoiRegion& first) noexcept {
		return static_cast<AABB::VoronoiRegion>(~static_cast<int>(first));
	}

	inline std::ostream& operator << (std::ostream& stream, const AABB& aabb) noexcept {
